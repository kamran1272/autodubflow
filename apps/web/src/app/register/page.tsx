...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!name.trim()) {
      setLocalError('A display name is required.');
      return;
    }
    ...
    try {
      ...
      setLocalError(authError ?? 'Unable to create account.');
    } finally {
  ...
  return (
    <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: '#07172d', color: '#edf5ff', padding: '24px' }}>
      <div style={{ width: '100%', maxWidth: 440, padding: 28, borderRadius: 20, background: 'rgba(11,17,31,0.88)', border: '1px solid rgba(160,176,216,0.15)', boxShadow: '0 20px 50px rgba(0,0,0,0.22)' }}>
        <h1 style={{ margin: '0 0 8px', fontSize: '2rem' }}>Create account</h1>
        <p style={{ margin: '0 0 20px', color: '#a2b7d7' }}>Register a secure AutoDubFlow account.</p>
        ...
        <p style={{ marginTop: 18, color: '#b8c8e4', fontSize: '0.94rem' }}>
